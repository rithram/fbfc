...
from cc_baseline import eval_cc_on_dataset_kfold_cv
from sbf_baseline import eval_sbf_on_dataset_kfold_cv
from fbfc_hpo import gp_hpo, get_blank_history
from lr_baseline import eval_lr_on_dataset_kfold_cv
from mlpc_baseline import eval_mlpc_on_dataset_kfold_cv
...
def get_all_methods(cmd_args):
    ...
    return [
        ...
        (
            'CC1',
            lambda d, n: eval_cc_on_dataset_kfold_cv(d, n, single=True)
        ),
        (
            'CC',
            lambda d, n: eval_cc_on_dataset_kfold_cv(d, n, single=False)
        ),
        (
            'SBFC',
        ...
        (
            'LR',
            lambda d, n: eval_lr_on_dataset_kfold_cv(d, n, nvals_for_C=20)
        ),
        (
            'MLPC',
            lambda d, n: eval_mlpc_on_dataset_kfold_cv(
                d, n, nvals_for_alpha=3, nvals_for_batch_size=4
            )
        ),
    ]
