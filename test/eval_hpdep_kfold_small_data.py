...
from fbfc_hpo import gen_pipeline_from_val, get_blank_history
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-c', '--nb_c_lb', help='Lower bound for \'c\' in the non-binary bloom filter', type=float,
    )
    parser.add_argument(
        '-C', '--nb_c_ub', help='Upper bound for \'c\' in the non-binary bloom filter', type=float,
    )
    parser.add_argument(
    ...
    c_hps = linspace(
        args.nb_c_lb, args.nb_c_ub, num=args.nvals_for_hp, endpoint=True
    ).tolist() if args.hp2tune == 'c' else [args.nb_c_lb, args.nb_c_ub]
    c_hps.append(None)
    if args.nb_c_ub == 1.0: c_hps.remove(1.0)
